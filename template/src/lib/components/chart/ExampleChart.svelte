	...
	const filteredData = $derived(
		chartData.filter((item) => {
			...
			referenceDate.setDate(
				referenceDate.getDate() - daysToSubtract
			)
...
<Card.Root
	class="bg-primary-foreground flex-1 overflow-hidden border-2 shadow-[0_0_15px_rgba(255,255,255,0.1)]">
	<Card.Header
		class="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
		<div
			class="grid flex-1 gap-1 text-center sm:text-left">
			<Card.Title
				>Area Chart - Interactive</Card.Title>
			<Card.Description
				>Showing total visitors for the last 3
				months</Card.Description>
		</div>
		<Select.Root
			type="single"
	...
	<Card.Footer>
		<div
			class="flex w-full items-start gap-2 text-sm">
